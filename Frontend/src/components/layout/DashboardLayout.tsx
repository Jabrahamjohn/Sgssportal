        ...
        <div className="px-5 py-6 border-b border-white/10">
          ...
          {me && (
            <p className="mt-2 text-[11px] text-gray-200">
              ...
              <span className="text-[var(--sgss-gold)]">
                ({me.role || "member"})
              </span>
            </p>
          )}
        ...
        <nav className="flex-1 px-3 py-4 overflow-auto space-y-5">
          ...
          <div>
            ...
            <Nav
              ...
              active={active("/dashboard/member/claims")}
              label="My Claims"
            />
            <Nav
              to="/dashboard/member/claims/new"
              icon={ShieldCheckIcon}
              active={active("/dashboard/member/claims/new")}
              label="New Claim"
            />
            <Nav
              to="/dashboard/member/chronic"
              icon={ShieldCheckIcon}
              active={active("/dashboard/member/chronic")}
              label="Chronic Illness"
            />
            <Nav
              to="/dashboard/member/profile"
              icon={UserCircleIcon}
              active={active("/dashboard/member/profile")}
              label="My Profile"
            />
            <Nav
              to="/dashboard/member/dependants"
              icon={UsersIcon}
              active={active("/dashboard/member/dependants")}
              label="Dependants"
            />
          </div>
          ...
          {isCommittee && (
            <div>
              ...
              <Nav
                ...
                icon={ClipboardDocumentListIcon}
                active={active("/dashboard/committee/claims")}
                label="All Claims"
              />
              {/* Pending view = claims with ?status=submitted */}
              <Nav
                to="/dashboard/committee/claims?status=submitted"
                icon={ShieldCheckIcon}
                active={
                  active("/dashboard/committee/claims") &&
              ...
              />
              <Nav
                to="/dashboard/committee/members"
                icon={UsersIcon}
                active={active("/dashboard/committee/members")}
                label="Members"
              />
            </div>
          )}
          ...
          {isAdmin && (
            <div>
              ...
              <Nav
                ...
                icon={ShieldCheckIcon}
                active={active("/dashboard/admin/audit")}
                label="Audit Logs"
              />
            </div>
          )}
