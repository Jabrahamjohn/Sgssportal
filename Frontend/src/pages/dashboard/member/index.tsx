    ...
    <PageTransition className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-r from-[var(--sgss-navy)] to-[#082e68] text-white shadow-xl p-8">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <Button
            ...
            className="bg-[var(--sgss-gold)] hover:bg-yellow-500 text-[var(--sgss-navy)] font-bold shadow-lg shadow-yellow-500/20 px-6 py-2.5 rounded-xl border-none"
          >
            + Create New Claim
          </Button>
        </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
             {role === "committee" && (
                <Link to="/dashboard/committee" className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/10 hover:bg-white/20 text-xs font-semibold backdrop-blur-md transition-colors border border-white/10">
                   <div className="w-2 h-2 rounded-full bg-[var(--sgss-gold)]"></div>
      ...
      <FadeIn delay={0.3}>
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 sgss-card p-0 overflow-hidden flex flex-col">
                      ...
                      {claims.slice(0, 5).map((c) => (
                        <tr key={c.id} className="hover:bg-gray-50/50 transition-colors group">
                          ...
                          <td className="px-6 py-3 text-right">
                             <Link to={`/dashboard/member/claims/${c.id}`} className="p-2 rounded-lg bg-gray-100 hover:bg-[var(--sgss-gold)] hover:text-white transition-colors inline-block text-gray-500">
          ...
          {/* PROFILE SUMMARY */}
          <div className="sgss-card h-fit">
             <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 rounded-full bg-[var(--sgss-navy)] text-white flex items-center justify-center text-lg font-bold">
                   {memberInfo?.full_name?.charAt(0) || "M"}
                </div>
                <div>
                   <h3 className="font-bold text-[var(--sgss-navy)]">{memberInfo?.full_name || "Guest Member"}</h3>
                   <p className="text-xs text-gray-500">{memberInfo?.email || "No Email"}</p>
                </div>
             </div>
