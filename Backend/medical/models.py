...
total = self.items.aggregate(sum=models.Sum(models.F('amount')*models.F('quantity')))['sum'] or 0
self.total_claimed = total
self.save(update_fields=['total_claimed'])


def compute_payable(self, reimbursement_scales, settings):
# simplified port of compute_claim_payable:
if self.excluded:
...
scale = s
break
fund_share_percent = scale.get('fund_share', 80) if scale else settings.get('fund_share_percent',80)
ceiling = scale.get('ceiling') if scale else settings.get('annual_limit',50000)
...
instance.compute_payable(scales, settings)